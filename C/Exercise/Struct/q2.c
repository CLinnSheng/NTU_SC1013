...
Complex mul(Complex c1, Complex c2){
    Complex new = {c1->real - c2->real, c1->imag - c2->imag};
    return new;    Complex new = {c1->real - c2->real, c1->imag - c2->imag};
    return new;    Complex new = {c1->real - c2->real, c1->imag - c2->imag};
    return new;
}

Complex div(Complex *c1, Complex *c2){
    Complex new = { (c1->real*c2->real + c1->imag*c2->imag) / (pow(c2->imag, 2) + pow(c2->real, 2)), (c1->imag*c2->real - c1->real*c2->imag) / (pow(c2->imag, 2) + pow(c2->real, 2)) };
    return new;
}